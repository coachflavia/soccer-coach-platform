...
import { DRILL_LIBRARY_EVENT, DRILL_LIBRARY_STORAGE_KEY, drillAgeGroups, drillCategories, emptyLibraryDrill, parseLibraryDrills, saveLibraryDrills, type LibraryDrill } from "../lib/drill-library-data";
...
export function LibraryDrillEditor({ drillId }: { drillId?: string }) {
  ...
  const [draft, setDraft] = useState<LibraryDrill>(() => existing ? structuredClone(existing) : emptyLibraryDrill());
  ...
  const save = () => { const next = { ...draft, name: draft.name.trim() || "Untitled drill", updatedAt: new Date().toISOString() }; saveLibraryDrills([...stored.filter(d => d.id !== next.id), next]); setDraft(next); setSaved(true); if (!drillId) router.replace(`/training/drills/${next.id}`); setTimeout(() => setSaved(false), 1800); };
  ...
  return <TrainingShell><div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
    ...
    <Link href="/training/drills" className="text-sm font-semibold text-slate-400">← Drill library</Link>
    <header className="mt-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-bold uppercase tracking-widest text-emerald-400">{drillId ? "Library drill" : "New library drill"}</p><h1 className="mt-2 text-3xl font-bold">{draft.name || "Create reusable drill"}</h1><p className="mt-2 text-sm text-slate-400">Personal library · Changes do not affect drills already added to sessions.</p></div><button onClick={save} className="training-primary">{saved ? "✓ Saved" : "Save Drill"}</button></header>
    <div className="mt-7 grid items-start gap-5 lg:grid-cols-[minmax(0,1.2fr)_minmax(360px,.8fr)]">
      ...
      <div className="space-y-5 lg:sticky lg:top-5">
        ...
        <section className="training-panel"><h2 className="text-xl font-bold">Organization</h2><Field label="Tags (comma separated)"><input className="training-input" value={draft.tags.join(", ")} onChange={e => update("tags", e.target.value.split(",").map(v => v.trim()).filter(Boolean))} placeholder="pressing, rondo, first touch"/></Field><fieldset className="mt-5"><legend className="training-label">Age groups</legend><div className="grid grid-cols-2 gap-2">{drillAgeGroups.map(age => <label key={age} className="flex min-h-11 items-center gap-2 rounded-lg bg-slate-800 px-3 text-sm"><input type="checkbox" checked={draft.ageGroups.includes(age)} onChange={() => toggleAge(age)} className="h-4 w-4 accent-emerald-500"/>{age}</label>)}</div></fieldset><p className="mt-5 text-xs text-slate-500">Ownership: Personal. Club and department ownership are reserved for a future backend migration.</p></section>
      </div>
    </div><div className="sticky bottom-3 mt-6 rounded-2xl border border-slate-700 bg-slate-900/95 p-3"><button onClick={save} className="training-primary w-full sm:w-auto">{saved ? "✓ Saved" : "Save Drill"}</button></div>
  </div></TrainingShell>;
}
